	...
	public class WaterMesh : MonoBehaviour
	{
		...
		{				
			...
			else
			{
				...
				// Initialize default assets by getting prefab references if not already initialized
				computeAsset = computeAsset != null ? computeAsset : (ComputeShader)AssetDatabase.LoadAssetAtPath("Assets/Ventuar/UnderwaterPack/PackAssets/Shaders/WaterCompute.compute", typeof(ComputeShader));
				waterMatAsset = waterMatAsset != null ? waterMatAsset : (Material)AssetDatabase.LoadAssetAtPath("Assets/Ventuar/UnderwaterPack/PackAssets/Materials/Water.mat", typeof(Material));
				foamNoiseAsset = foamNoiseAsset != null ? foamNoiseAsset : (Texture2D)AssetDatabase.LoadAssetAtPath("Assets/Ventuar/UnderwaterPack/PackAssets/Textures/Foam Noise.png", typeof(Texture2D));
				foamFlowAsset = foamFlowAsset != null ? foamFlowAsset : (Texture2D)AssetDatabase.LoadAssetAtPath("Assets/Ventuar/UnderwaterPack/PackAssets/Textures/Foam Distortion Flow Map.png", typeof(Texture2D));

				// Assign default values to specific fields if null
				if (computeShader == null)
					computeShader = computeAsset;
					
				if (rend.sharedMaterial == null)
					rend.sharedMaterial = waterMatAsset;

				if (foamNoise == null)
					foamNoise = foamNoiseAsset;

				if (foamDistortionTexture == null)
					foamDistortionTexture = foamFlowAsset;

				// Typically happens when water is duplicated
